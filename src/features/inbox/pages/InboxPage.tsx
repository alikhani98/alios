...
export function InboxPage() {
  ...
  const run = async (id: string, action: () => Promise<void>, success: string, failure = t("inbox.actionError")) => {
  ...
  return (
    <section className="alios-page space-y-6">
      ...
      {isLoading ? <div className="grid gap-4 md:grid-cols-2" aria-label={t("inbox.loading")}>{[0, 1].map((i) => <div key={i} className="h-48 animate-pulse rounded-2xl border bg-muted/60" />)}</div> : items.length === 0 ? <Card className="border-dashed"><CardContent className="flex flex-col items-center px-6 py-14 text-center"><div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Inbox className="h-6 w-6" /></div><h3 className="text-lg font-semibold">{t("inbox.emptyTitle")}</h3><p className="mt-2 max-w-md text-sm leading-7 text-muted-foreground">{t("inbox.emptyDescription")}</p></CardContent></Card> : <div className="grid gap-4 md:grid-cols-2">{items.map((item) => <InboxItemCard key={item.id} item={item} isBusy={busyId === item.id} onEdit={async (values) => { let saved = false; await run(item.id, async () => { await updateItem(item.id, values); saved = true; }, t("inbox.updated")); return saved; }} onToggleStatus={() => run(item.id, () => (item.status === "unprocessed" ? markProcessed(item.id) : markUnprocessed(item.id)).then(() => undefined), t("inbox.statusUpdated"))} onConvert={(target) => run(item.id, () => convertItem(item.id, target).then(() => undefined), t("inbox.conversionSuccess"), t("inbox.conversionFailure"))} onDelete={() => run(item.id, () => deleteItem(item.id), t("inbox.deleted"))} />)}</div>}
    </section>
  );
}
