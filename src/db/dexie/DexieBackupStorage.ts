...
export class DexieBackupStorage implements BackupStorage {
  ...
  async readAll(): Promise<AliosBackupData> {
    try {
      const [
        ...
        this.database.financeCategoryBudgets.toArray(),
        this.database.financeAssets.toArray(),
        this.database.focusSessions.toArray(),
  ...
  async replaceAll(data: AliosBackupData): Promise<void> {
    const tables = [
      ...
      this.database.financeCategoryBudgets,
      this.database.financeAssets,
    ...
    try {
      await this.database.transaction("rw", tables, async () => {
        ...
        await Promise.all([
          ...
          this.database.financeCategoryBudgets.bulkPut(data.financeCategoryBudgets),
          this.database.financeAssets.bulkPut(data.financeAssets),
          this.database.focusSessions.bulkPut(data.focusSessions),
          ...
          this.database.resources.bulkPut(data.resources),
  ...
  async getSummary(): Promise<LocalDataSummary> {
    try {
      const [
        ...
        this.database.financeCategoryBudgets.count(),
        this.database.financeAssets.count(),
        this.database.focusSessions.count(),
