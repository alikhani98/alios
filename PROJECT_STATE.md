...
- Current Stage: Stage 85 Completed (Stage 84 manual usage pass remains pending)
...
Stage 85 completion is evidenced by explicit cacheable Vite vendor chunks, direct startup imports that exclude forms/validation code from the entry module-preload list, and `docs/PERFORMANCE.md`. On 2026-07-17 the measured primary entry decreased from 550.15 kB to 239.01 kB; the 82.28 kB forms vendor chunk is no longer preloaded by `index.html`, and Vite emits no chunk-size warning. These are build-output measurements rather than real-user timing claims. The stage changes no route, runtime product behavior, data, repository contract, Dexie schema, backup format/version, dependency, backend, sync, cloud, AI, telemetry, or user-data behavior.
...
Next stage: complete the Stage 84 real-world usage pass and the Stage 85 deployed/throttled initial-load smoke test before approving additional product scope.
...
- Production build: no Vite chunk-size warning; primary entry is 239.01 kB, with forms/validation code excluded from entry module-preloads
