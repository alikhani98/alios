...
- Settings now exposes a bilingual, accessible View density / تراکم نمایش radio group in the appearance/interface area. The control is browser-local and does not touch backup, sync, schema, route, repository, or product data.
...
- The live GitHub Pages site returned HTTP 200 and served the Stage 156 entry asset `index-COZnbLy8.js`; the deployed Settings chunk contains `alios.viewDensityMode`, `View density`, and `تراکم نمایش`.
...
- Evidence includes a user-provided screenshot from Decision Log / دفترچه تصمیم‌ها in Simple View showing the large introductory help panel and the proposed smaller contextual help affordance.
- `docs/CONTEXTUAL_HELP_PATTERN.md` now defines a documentation-only lightweight Help / راهنما / info-icon pattern near page or section intros, with hover, keyboard focus, click, touch, Escape, outside-click, focus-visible, 200% zoom, RTL/LTR, and 360/390/430 px mobile requirements.
- The next valid product stage is a small contextual help pilot on one or two pages, likely Decision Log / دفترچه تصمیم‌ها, Personal Manual, or Goals. It must not be a broad redesign or app-wide rollout.
...
- Decision Log / دفترچه تصمیم‌ها now uses the existing `viewDensityMode` preference to make the page intro lighter in Simple View only.
- In Simple View, the large always-visible explanatory text is replaced by a compact Help / راهنما button with button semantics, `aria-expanded`, `aria-controls`, visible shared focus styling, and an inline disclosure that works by click, tap, and keyboard activation.
...
- Personal Manual and Goals now use the existing `viewDensityMode` preference to replace the large always-visible Simple View intro guidance with a compact Help / راهنما button and inline disclosure near the page intro.
